...
async function main() {
  ...
  console.log(`Found ${chunks.length} entries to restore.`);
  
  for (const chunk of chunks) {
    const lines = chunk.split('\n').map(l => l.trim()).filter(l => l.length > 0 && l !== 'WhatsApp Pass' && l !== 'Email Pass');
    if (lines.length < 7) continue;
    
    // The format seems to be: Name, Dept, Email, Phone, Likelihood, Date, Status
    const name = lines[0];
    const dept = lines[1];
    const email = lines[2];
    const phone = lines[3];
    const likelihood = lines[4];
    const dateStr = lines[5]; // e.g. "Jul 31, 3:03 PM"
    const status = lines[6] ? lines[6].toLowerCase() : 'approved';
    ...
    try {
      await sqlite.execute({
        sql: `INSERT INTO attendance_requests (
          event_id, name, phone, email, department, attendance_possibility, status, ticket_code, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [
          1,
          name,
